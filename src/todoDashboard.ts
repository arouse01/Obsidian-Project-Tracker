...
export class TodoDashboardView extends ItemView {
	...
	async updateProjects(): Promise<void> {
		...
		const activeTodos = await this.todoManager.getActiveTodos();
		const activeTodoMap = new Map(
			activeTodos.map(todo => [todo.projectPath, todo])
		);
		...
		for (const project of projects) {
			...
			const activeSession = activeTodoMap.get(project.file.path);
